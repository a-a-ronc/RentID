...
import { AppShell, ListRow, PageHeader, SectionCard, TrustBadge, VerificationChecklist } from "@/components/rentid/patterns";
...
export const Route = createFileRoute("/_authenticated/tenant/tenancy")({
  head: () => ({ meta: [{ title: "My tenancy — RentID" }, { name: "robots", content: "noindex" }] }),
...
function TenancyPage() {
  ...
  return (
    <AppShell subtitle="Tenant">
      ...
      {tenancies.isLoading ? (
        <div className="mt-5"><EmptyState title="Loading…" description="Fetching your tenancy." /></div>
      ) : !active ? (
        <div className="mt-5"><EmptyState title="No tenancy yet" description="Your tenancy will appear here once a landlord invites you." /></div>
      ) : (
        <>
          <Glass className="mt-5 p-5">
            <Eyebrow>{active.organization?.name ?? "Landlord"}</Eyebrow>
            <h2 className="mt-1 font-display text-[18px] font-semibold">{active.property?.name} {active.unit?.name}</h2>
            <p className="mt-1 text-[12.5px] text-muted-foreground">
              {active.property?.street_address}, {active.property?.city} {active.property?.state}
            </p>
            <div className="mt-3">
              {active.verified ? <TrustBadge kind="verified_tenancy" /> : <TrustBadge kind="unverified" />}
            </div>
          </Glass>
          ...
          <SectionCard title="Landlord contact" className="mt-4">
            <ListRow title={active.organization?.name ?? "Landlord"} subtitle="Message them from the Messages tab" />
          </SectionCard>
        </>
      )}
    </AppShell>
