...
function PreLeasingPage() {
  ...
  if (!s) {
    return (
      <AppShell subtitle="Property manager" role="manager">
        <EmptyState title="No student properties" description="Turn on the student category to start pre-leasing." />
      </AppShell>
    );
  }
  ...
  return (
    <AppShell subtitle="Property manager" role="manager">
      <PageHeader
        ...
        subtitle={s.term ? `Next term ${s.term.label} · move-in ${s.term.move_in_on}` : "Academic term pipeline"}
      />

      <SummaryGrid
        className="mt-4"
        items={[
          { label: "Pre-leased", value: `${s.pre_leased_pct}%`, hint: `${s.pre_leased} of ${s.beds_total} beds` },
      ...
      <div className="mt-4">
        <DemoNotice>
          Roommate groups apply together but stay separate people: each member has their own profile,
          guarantor and payment obligation. First-time renters are shown as a first verified tenancy —
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1fr_1.3fr]">
        <Glass className="p-4">
          ...
          <div className="mt-3 space-y-2">
            {s.pipeline.map((row) => (
              <div key={row.status} className="flex items-center justify-between gap-3 rounded-xl bg-secondary/50 px-3 py-2">
        ...
        <SectionCard title="Roommate groups" aside={`${s.groups.length} groups`}>
          {s.groups.length === 0 ? (
            <EmptyState title="No groups yet" description="Group applications for the next term will appear here." />
          ) : (
            s.groups.map((g) => (
              <div key={g.id} className="px-4 py-3">
                ...
                <div className="mt-2 space-y-1.5">
                  {g.members.map((m) => (
                    <div key={m.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-secondary/50 px-3 py-2">
                      <span className="text-[12.5px]">{m.name}</span>
                      <div className="flex flex-wrap gap-1.5">
                        {m.first_time_renter ? <Pill>First verified tenancy</Pill> : <Pill>Rental history shared</Pill>}
                        <StatusPill
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <Button tone="secondary" onClick={() => advance.mutate({ groupId: g.id, stage: "screening" })}>
                    Move to screening
                  </Button>
                  <Button tone="secondary" onClick={() => advance.mutate({ groupId: g.id, stage: "approved" })}>
                    Approve group
                  </Button>
                  <Button tone="ghost" onClick={() => advance.mutate({ groupId: g.id, stage: "waitlisted" })}>
                    Waitlist
                  </Button>
                </div>
              </div>
