...
import { Button, DemoNotice, InlineError, LoadingCard, Modal, TextInput, Field } from "@/components/rentid/kit";
...
function LeaseChangeInbox() {
  ...
  return (
    <AppShell subtitle="Property manager" role="manager">
      ...
      <div className="mt-4">
        <DemoNotice>
          Requests follow the approval-first rule: a prohibited request can still be submitted for an
          exception, but it never becomes effective — and no replacement listing goes public — without
      ...
      <div className="mt-4">
        <SectionCard title="Requests" aside={`${rows.length} shown`}>
          {requests.isLoading ? (
            ...
            <div className="p-4">
              <InlineError message="Could not load lease changes." onRetry={() => requests.refetch()} />
            </div>
          ) : rows.length === 0 ? (
            <EmptyState title="No lease change requests" description="Resident requests will appear here for review." />
          ) : (
            rows.map((r) => (
              <button
                key={r.id}
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="num mt-0.5 truncate text-[11.5px] text-muted-foreground">
                    ...
                    {r.bed_label ? ` · Bed ${r.bed_label}` : ""} · {r.checklist.filter((c) => c.done).length}/
                    {r.checklist.length} steps complete
                  </p>
                </div>
...
function RequestDrawer({
  ...
  const ownerApproved = !request.requires_owner_approval || request.steps.some((s) => s.role === "owner" && s.state === "approved");
      ...
      <div className="space-y-4">
        <Glass className="p-3">
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            <Pill>{request.replacement_listing_enabled ? "Replacement listing live" : "No public listing"}</Pill>
          </div>
        </Glass>
        ...
        <div>
          ...
          <div className="mt-2 space-y-1.5">
            {request.steps.map((s) => (
              <div key={s.id} className="flex items-center justify-between gap-3 rounded-xl bg-secondary/50 px-3 py-2">
                <span className="text-[12.5px]">{s.label}</span>
                <StatusPill
                  status={s.state}
                  tone={s.state === "approved" ? "success" : s.state === "denied" ? "danger" : "warning"}
                />
              </div>
            ))}
          </div>
          ...
          <ul className="mt-2 space-y-1">
            {request.checklist.map((c) => (
              <li key={c.label} className="flex items-center gap-2 text-[12.5px]">
                <span className={c.done ? "text-success" : "text-muted-foreground"}>{c.done ? "●" : "○"}</span>
        ...
        {!decided ? (
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {request.state === "submitted" ? <Button onClick={() => act("start_review")}>Start review</Button> : null}
              {!pmApproved ? <Button onClick={() => act("pm_approve")}>Approve as manager</Button> : null}
              {pmApproved && !ownerApproved ? <Button onClick={() => act("owner_approve")}>Record owner approval</Button> : null}
              ...
              {request.request_type === "replacement_resident" && pmApproved && !request.replacement_listing_enabled ? (
                <Button tone="secondary" onClick={() => act("enable_replacement_listing")}>
                  Allow replacement listing
                </Button>
              ) : null}
              {request.state === "scheduled" ? <Button onClick={() => act("make_effective")}>Make effective</Button> : null}
